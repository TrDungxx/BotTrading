...
} from "./popupchart/LineIndicatorSettings";
...
function calculateMA(
  ...
  for (let i = period - 1; i < data.length; i++) {
    ...
    if (!Number.isNaN(avg))
      out.push({ time: data[i].time, value: +avg.toFixed(8) });
  }
  return out;
}
...
async function getSymbolMeta(
  ...
  const PRICE_FILTER = (info.filters || []).find(
...
const TradingBinance: React.FC<Props> = ({
  ...
  const [bollFillVisible, setBollFillVisible] = useState(
    savedSettings?.bollFillVisible ?? false
  ); // For BOLL fill background
  ...
  const [indicatorPeriods, setIndicatorPeriods] = useState(
    savedSettings?.indicatorPeriods ?? {
      ma7: 7,
      ma25: 25,
      ma99: 99,
      ema12: 12,
  ...
  const [floatingPos, setFloatingPos] = useState<PositionForTag | undefined>(
    undefined
  );
  ...
  const snapToTick = (price: number) => {
    ...
    if (price < 10 && tick >= 0.05)
      tick = heuristicMetaFromPrice(price).tickSize;
  ...
  useEffect(() => {
    ...
    if (candles.length && candleSeries.current) {
      ...
      const needsBarData = ["Bars", "High-low"].includes(chartType);

      if (needsLineData) {
        const lineData = candles.map((c) => ({ time: c.time, value: c.close }));
        (candleSeries.current as unknown as ISeriesApi<"Line">).setData(
          lineData as any
        );
        ...
        (candleSeries.current as unknown as ISeriesApi<"Histogram">).setData(
          histData as any
        );
        ...
        (candleSeries.current as unknown as ISeriesApi<"Bar">).setData(
          barData as any
        );
      } else {
  ...
  const updatePriceFormat = async (candles: Candle[]) => {
    ...
    cs.applyOptions({
      priceFormat: { type: "price", minMove: displayTickSize, precision },
    });
    ma7Ref.current?.applyOptions({
      priceFormat: { type: "price", minMove: displayTickSize, precision },
    });
    ma25Ref.current?.applyOptions({
      priceFormat: { type: "price", minMove: displayTickSize, precision },
    });
    ma99Ref.current?.applyOptions({
      priceFormat: { type: "price", minMove: displayTickSize, precision },
    });
  ...
  function openCtxMenu(e: React.MouseEvent<HTMLDivElement>) {
    ...
    if (series) {
      ...
      ctxClickPriceRef.current =
        pAtOpen != null && Number.isFinite(pAtOpen as number)
          ? (pAtOpen as number)
          : null;
    } else {
    ...
    if (chart && series) {
      if (hoverPrice == null) {
        ...
        setHoverPrice(
          priceAtY != null && Number.isFinite(priceAtY as number)
            ? (priceAtY as number)
            : null
        );
      }
      const timeAtX = chart
        .timeScale()
        .coordinateToTime(clickX) as UTCTimestamp | null;
      ...
      try {
        ...
        if (vs.length >= indicatorPeriods.mavol1 && mavol1Ref.current) {
          mavol1Ref.current.setData(
            calculateVolumeMA(vs, indicatorPeriods.mavol1)
          );
        }
        if (vs.length >= indicatorPeriods.mavol2 && mavol2Ref.current) {
          mavol2Ref.current.setData(
            calculateVolumeMA(vs, indicatorPeriods.mavol2)
          );
        }
        ...
        if (cs.length > 0) {
          setTimeout(() => {
            ...
            const containerWidth =
              mainChartContainerRef.current?.clientWidth || 800;
            const minBarWidth = 8;
            ...
            let targetVisible = Math.max(
              50,
              Math.min(100, Math.floor((minCandles + maxCandles) / 2))
            );
            ...
            const optimalBarSpacing = Math.max(
              8,
              Math.min(16, Math.floor((containerWidth / visibleCount) * 0.85))
            );
            ...
            console.log(
              `[InitialView] 📊 ${visibleCount}/${cs.length} nến (spacing=${optimalBarSpacing}px)`
            );
          }, 150);
        }
        ...
        if (cs.length >= 7 && ma7Ref.current)
          ma7Ref.current.setData(calculateMA(cs, 7));
        if (cs.length >= 25 && ma25Ref.current)
          ma25Ref.current.setData(calculateMA(cs, 25));
        if (cs.length >= 99 && ma99Ref.current)
          ma99Ref.current.setData(calculateMA(cs, 99));
        // ✅ ADD EMA calculations
        if (cs.length >= 12 && ema12Ref.current && mainVisible.ema12) {
          ema12Ref.current.setData(
            calculateEMA(cs, indicatorPeriods.ema12 || 12)
          );
        }
        if (cs.length >= 26 && ema26Ref.current && mainVisible.ema26) {
          ema26Ref.current.setData(
            calculateEMA(cs, indicatorPeriods.ema26 || 26)
          );
        }
        ...
        const ws = new WebSocket(
          `${wsBase}/${selectedSymbol.toLowerCase()}@kline_${selectedInterval}`
        );
        wsRef.current = ws;
        ...
        ws.onmessage = (ev) => {
          ...
          if (candleSeries.current && volumeSeries.current) {
            ...
            setVolumeData((prev) => {
              ...
              if (next.length >= indicatorPeriods.mavol1 && mavol1Ref.current) {
                mavol1Ref.current.setData(
                  calculateVolumeMA(next, indicatorPeriods.mavol1)
                );
              }
              if (next.length >= indicatorPeriods.mavol2 && mavol2Ref.current) {
                mavol2Ref.current.setData(
                  calculateVolumeMA(next, indicatorPeriods.mavol2)
                );
              }
          ...
          setCandles((prev) => {
            ...
            if (next.length >= 7 && ma7Ref.current)
              ma7Ref.current.setData(calculateMA(next, 7));
            if (next.length >= 25 && ma25Ref.current)
              ma25Ref.current.setData(calculateMA(next, 25));
            if (next.length >= 99 && ma99Ref.current)
              ma99Ref.current.setData(calculateMA(next, 99));

            // ✅ ADD: Update EMA
            if (
              next.length >= (indicatorPeriods.ema12 || 12) &&
              ema12Ref.current &&
              mainVisible.ema12
            ) {
              ema12Ref.current.setData(
                calculateEMA(next, indicatorPeriods.ema12 || 12)
              );
            }
            if (
              next.length >= (indicatorPeriods.ema26 || 26) &&
              ema26Ref.current &&
              mainVisible.ema26
            ) {
              ema26Ref.current.setData(
                calculateEMA(next, indicatorPeriods.ema26 || 26)
              );
            }

            // ✅ ADD: Update BOLL and redraw canvas
            if (
              next.length >= (indicatorPeriods.boll?.period || 20) &&
              mainVisible.boll
            ) {
      ...
      try {
        if (candleSeries.current) {
          ...
          localStorage.setItem(
            hlineKey(selectedSymbol, market),
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("chart-symbol-change-request", handler);
    ...
    <div className="h-full w-full min-w-0 relative">
      ...
      {mainVisible.boll && (
        <BollingerBandsIndicator
          ...
          colors={{
            upper: indicatorColors.boll?.upper || "#B385F8",
            middle: indicatorColors.boll?.middle || "#EB40B5",
            lower: indicatorColors.boll?.lower || "#B385F8",
            fill: indicatorColors.boll?.fill || "rgba(179, 133, 248, 0.1)",
          }}
        ...
        <LineIndicatorSettings
          ...
          onChange={(mainVis, volumeVis, _, per, col, bollFillVis) => {
            ...
            if (per && candles.length > 0) {
              ...
              if (
                candles.length >= (per.ema12 || 12) &&
                ema12Ref.current &&
                mainVis?.ema12
              ) {
                ema12Ref.current.setData(
                  calculateEMA(candles, per.ema12 || 12)
                );
              }
              if (
                candles.length >= (per.ema26 || 26) &&
                ema26Ref.current &&
                mainVis?.ema26
              ) {
                ema26Ref.current.setData(
                  calculateEMA(candles, per.ema26 || 26)
                );
              }
            ...
            if (per && volumeData.length > 0) {
              if (
                volumeData.length >= (per.mavol1 || 7) &&
                mavol1Ref.current &&
                volumeVis?.mavol1
              ) {
                mavol1Ref.current.setData(
                  calculateVolumeMA(volumeData, per.mavol1 || 7)
                );
              }
              if (
                volumeData.length >= (per.mavol2 || 14) &&
                mavol2Ref.current &&
                volumeVis?.mavol2
              ) {
                mavol2Ref.current.setData(
                  calculateVolumeMA(volumeData, per.mavol2 || 14)
                );
              }
            }
          }}
          ...
          <div className="py-2 text-fluid-sm text-dark-100">
            ...
            <button
              ...
              onMouseDown={(e) => {
                ...
                if (
                  ctxClickPriceRef.current != null &&
                  Number.isFinite(ctxClickPriceRef.current)
                ) {
                ...
                if (!series || price == null || !Number.isFinite(price)) {
                  console.warn(
                    "[HLINE] missing series/price after all fallbacks",
                    {
                      seriesOk: !!series,
                      price,
                    }
            ...
            <button className="w-full px-fluid-4 py-2 text-left hover:bg-dark-700 flex items-center gap-fluid-3">
              <span className="i-lucide-sliders-horizontal shrink-0" /> Xóa chỉ
              báo
            </button>
      ...
      <NewOrderModal
        ...
        tickSize={
          symbolMetaCache.get(`${market}:${selectedSymbol.toUpperCase()}`)
            ?.tickSize
        }
        pricePrecision={
          symbolMetaCache.get(`${market}:${selectedSymbol.toUpperCase()}`)
        ...
        onSubmit={(p) => {
          const meta = symbolMetaCache.get(
            `${market}:${selectedSymbol.toUpperCase()}`
          );
          const step = meta?.stepSize ?? 0.00000001;
          const stepDec = String(step).includes(".")
      ...
      <div className="relative w-full h-full flex flex-col">
        ...
        <div ref={volumeChartContainerRef} className="relative w-full flex-1">
              ...
              onChange={(mainVis, volumeVis, _, per, col, bollFillVis) => {
                ...
                if (volumeVis) {
                  ...
                  mavol1Ref.current?.applyOptions({
                    visible: volumeVis.mavol1,
                  });
                  mavol2Ref.current?.applyOptions({
                    visible: volumeVis.mavol2,
                  });
                }
                if (per) setIndicatorPeriods(per);
                if (col) {
                ...
                if (per && candles.length > 0) {
                  ...
                  if (
                    ...
                    ma25Ref.current.setData(
                      calculateMA(candles, per.ma25 || 25)
                    );
                  }
                  if (
                    candles.length >= (per.ma99 || 99) &&
                    ...
                    ma99Ref.current.setData(
                      calculateMA(candles, per.ma99 || 99)
                    );
                  }
                  if (
                    candles.length >= (per.ema12 || 12) &&
