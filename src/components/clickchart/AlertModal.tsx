...
const AlertModal: React.FC<Props> = ({ open, onClose, defaultPrice, symbol, onCreate }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50">
      <div className="w-[360px] rounded-xl border border-dark-600 bg-dark-800 p-fluid-4">
        <div className="text-lg font-semibold mb-3">Tạo cảnh báo • {symbol}</div>
        <label className="text-fluid-sm text-dark-300">Giá</label>
        <input
          ...
          placeholder="0.00"
        />
        <label className="mt-3 block text-fluid-sm text-dark-300">Ghi chú</label>
        <input
          className="mt-1 w-full rounded-lg bg-dark-700 px-fluid-3 py-2 outline-none"
          value={note}
