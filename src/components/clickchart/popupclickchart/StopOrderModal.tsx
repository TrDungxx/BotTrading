...
const StopOrderModal: React.FC<StopOrderModalProps> = ({
  open,
  onClose,
  symbol,
  defaultPrice,
  ...
  const [orderType, setOrderType] = useState<'limit' | 'stopLimit'>('stopLimit');
  const [stopPrice, setStopPrice] = useState('');
  const [price, setPrice] = useState('');
  const [quantity, setQuantity] = useState('');
  const [sliderValue, setSliderValue] = useState(0);
  const [workingType, setWorkingType] = useState<'MARK' | 'LAST'>('MARK');
  const [showWorkingTypeDropdown, setShowWorkingTypeDropdown] = useState(false);

  // Lấy tên coin từ symbol
  ...
  const handleSubmit = (side: 'BUY' | 'SELL') => {
    ...
    if (isNaN(stopPriceNum) || isNaN(priceNum) || isNaN(qtyNum) || qtyNum <= 0) {
      return;
    }
            ...
            <div className="flex">
              ...
              <div className="relative">
                <button 
                  ...
                  onClick={() => setShowWorkingTypeDropdown(!showWorkingTypeDropdown)}
                >
                  {workingType === 'MARK' ? 'Mark' : 'Last'}
                  <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                  </svg>
                </button>
                {showWorkingTypeDropdown && (
                  <div className="absolute right-0 top-full mt-1 bg-[#2b3139] border border-[#3c4043] rounded shadow-lg z-10">
                    <button
                      className="w-full px-fluid-4 py-2 text-fluid-sm text-[#eaecef] hover:bg-[#3c4043] text-left"
                      onClick={() => {
                        setWorkingType('MARK');
          ...
          <div className="mb-3">
            ...
            <div className="flex">
              <input
                ...
                placeholder="0"
              />
              <button className="px-fluid-3 py-2.5 bg-[#2b3139] border border-l-0 border-[#3c4043] rounded-r text-fluid-sm text-[#eaecef] flex items-center gap-fluid-1">
                {coinName}
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
