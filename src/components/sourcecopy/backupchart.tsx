...
const INDICATOR_SETTINGS_VERSION = '2.0'; // ✅ Version to force reset old settings
  ...
  try {
    ...
    if (saved) {
      ...
      if (parsed.version !== INDICATOR_SETTINGS_VERSION) {
        console.log('[Indicators] Clearing old settings');
...
useEffect(() => {
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      if (cs.length >= 7 && ma7Ref.current && mainVisible.ma7) ma7Ref.current.setData(calculateMA(cs, 7));
      if (cs.length >= 25 && ma25Ref.current && mainVisible.ma25) ma25Ref.current.setData(calculateMA(cs, 25));
      if (cs.length >= 99 && ma99Ref.current && mainVisible.ma99) ma99Ref.current.setData(calculateMA(cs, 99));
      ...
      ws.onmessage = (ev) => {
        ...
        setCandles((prev) => {
          ...
          if (next.length >= 7 && ma7Ref.current && mainVisible.ma7) ma7Ref.current.setData(calculateMA(next, 7));
          if (next.length >= 25 && ma25Ref.current && mainVisible.ma25) ma25Ref.current.setData(calculateMA(next, 25));
          if (next.length >= 99 && ma99Ref.current && mainVisible.ma99) ma99Ref.current.setData(calculateMA(next, 99));
...
{showMainSettings && createPortal(
    ...
    onChange={(mainVis, volumeVis, _, per, col, bollFillVis) => {
      ...
      if (per && candles.length > 0) {
        if (candles.length >= (per.ma7 || 7) && ma7Ref.current && mainVis?.ma7) {
          if (mainVis?.ma7) ma7Ref.current.setData(calculateMA(candles, per.ma7 || 7));
        }
        if (candles.length >= (per.ma25 || 25) && ma25Ref.current && mainVis?.ma25) {
          if (mainVis?.ma25) ma25Ref.current.setData(calculateMA(candles, per.ma25 || 25));
        }
        if (candles.length >= (per.ma99 || 99) && ma99Ref.current && mainVis?.ma99) {
          if (mainVis?.ma99) ma99Ref.current.setData(calculateMA(candles, per.ma99 || 99));
        }
  ...
  containerRef={mainChartContainerRef}  // ✅ ĐÚNG - mainChartContainerRef được attach ở line 2569
  offset={12}
/>
